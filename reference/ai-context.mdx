...
description: "Use Dollr docs with LLMs — llms.txt, OpenAPI, documentation MCP, and support checklist."
keywords: ["llms.txt", "MCP", "AI agents", "OpenAPI", "Dollr documentation"]
...
The docs site includes Mintlify’s **Assistant** panel on the right (sparkle icon in the header, or **Ask AI** in the sidebar anchors). It answers from published Dollr documentation.

- **Open on load:** Pages use `?assistant=open` and a small site script so the panel opens by default; readers can **close** or **expand** it. Closing it once keeps it closed for that browser session.
- **Enable in production:** Turn on **Assistant** in [Mintlify Dashboard → Assistant](https://dashboard.mintlify.com/products/assistant) for the connected `dollr_api_docs` project. Configure starter questions (for example: “How do I collect with an order instead of an invoice?”).
- **Keyboard shortcut:** ⌘I (macOS) / Ctrl+I (Windows).

The contextual menu includes **Assistant** alongside Copy page, View, Claude, and ChatGPT.
